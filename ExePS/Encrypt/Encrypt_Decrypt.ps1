param([string]$psFile,[string]$key,[string]$action);
...
function openDialog ($fileType) {
    $dateNow = (get-date).ToString('yyyy_MM_dd_hh_mm_ss')

    $openDiag = New-Object -Typename System.Windows.Forms.OpenFileDialog 
    $openDiag.initialDirectory = "$dir"
    if($fileType -eq "PS"){
        $openDiag.filter = "PowerShell Files|*.ps1" 
    } elseif($fileType -eq "TXT"){
        $openDiag.filter = "TXT Files|*.txt"
    }
    $openDiag.ShowDialog()
...
try{
    $get = get-content $psFile -raw -Encoding UTF8
} catch {
    write-host $_
}
...
if($action -eq "protect"){
    ...
    if(!($psFile)){
        $openDiag = openDialog -fileType "PS"

        if($openDiag[0] -eq "OK"){
            $psFile = $openDiag.FileName
        } else {
            rite-host "Open File canceled. Rerun script."
        }
    }

    try{
        $protect = Protect-String -String $get -Key $key 
    } catch {
        write-host $_
    }

    $outFileDetination = $psFile -replace ".ps1", ".txt"

    $protect | out-file $outFileDetination

} elseif($action -eq "unprotect"){
    if(!($psFile)){
        $openDiag = openDialog -fileType "TXT"

        if($openDiag[0] -eq "OK"){
            $psFile = $openDiag.FileName
        } else {
            write-host "Open File canceled. Rerun script."
        }
    }

    try{
        $unProtect = Unprotect-String -ProtectedString $get -Key $key 
    } catch {
        write-host $_
    }

    $outFileDetination = $psFile -replace ".txt", ".ps1"

    $unProtect | out-file $outFileDetination
